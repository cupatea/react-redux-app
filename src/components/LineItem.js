...
const LineItem = ({ classes, image, title, currentQuantity, sizes, price, handleDelete, handleIncrement, handleDecrement })=>(
	<div className = { classes.root }>
		<img className = { classes.image} src = { serverURL(image) } alt = { title }/>
		<div className = { classes.flex }>
			<div className = { classes.title }><Typography type = 'body2' children = { title } /></div>
			<div className = { classes.detail }>
				<div className = { classes.quantity }>
					...
					<IconButton className = { classes.quantityButton } aria-label = "Add" onClick = { () => handleIncrement() }>
						<Add className = { classes.icon } />
					</IconButton> 
    
				</div>
				<div className = { classes.sizes }>
					{ Object.entries(sizes).map( size => <Typography key = { size[0] } className = { classes.size } type = 'caption' children = { size.join(' size: ') }/>) }
				</div>
